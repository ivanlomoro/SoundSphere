...
export interface UserInteractionProps {
    ...
    playlists: Playlist[]
    selectedSongs: Songs[]
    createNewPlaylist: (playlist: Playlist, playlistName: Playlist['playlistName']) => void
    updatePlaylist: (frontId: Playlist["frontId"], playlistName: Playlist['playlistName']) => void
...
const UserInteractionProvider: React.FC<ProviderProps> = ({ children }) => {
    ...
    const isLiked = (frontId: Playlist['frontId']): boolean => playlists.some(playlist => playlist.frontId === frontId)
    const playlistExists = (arr: Playlist[], playlistFrontId: string) => arr.some(playlist => playlist.frontId === playlistFrontId)
    const addToPlaylists = (playlist: Playlist) => !playlistExists(playlists, playlist.frontId) && setPlaylists(prev => [playlist, ...prev])
    const removeFromPlaylists = (frontId: Playlist['frontId']) => {setPlaylists(prev => prev.filter(playlist => playlist.frontId !== frontId)) ;setSelectedPlaylist(playlists[0] || placeHolderPlaylist )}
    ...
    const addToSelected = (song: Songs) => {
        if (selectedPlaylist && selectedPlaylist != placeHolderPlaylist) {
            ...
            setPlaylists(prevPlaylists =>
                prevPlaylists.map(p => p.frontId === updatedPlaylist.frontId ? updatedPlaylist : p));
    ...
    const createNewPlaylist = (playlist: Playlist, newName: Playlist['playlistName']) => {
            ...
            setSelectedSongs(currentSelectedSongs => {
                ...
                if (selectedPlaylist && selectedPlaylist.songs.some(song => song.id === id)) {
                    ...
                    if (updatedPlaylist.songs.length === 0) {
                        setPlaylists(currentPlaylists => currentPlaylists.filter(playlist => playlist.frontId !== selectedPlaylist.id));
                        playlists ? setSelectedPlaylist(playlists[0]) : setSelectedPlaylist(placeHolderPlaylist);
                        setSelectedSongs([]);
                    }
                }
    ...
    return (
        <InteractionContext.Provider value={{ selectedPlaylist, updatePlaylist, createNewPlaylist, removeFromPlaylists, playlists, isSelected, selectedSongs, addToSelected, toggleSelected, recents, favorites, addToRecents, isFavorite, toggleFavorite, toggleLiked, isLiked }}>
            {children}
